...
export default function AdminPaymentDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  // Billing-based proof → group billing doc; PO-based proof → the PO itself carries the billing
  const billingItem = proof.billingId
    ? { _id: proof.billingId, type: "group" as const, billingNumber: proof.billingNumber }
    : proof.poId
      ? { _id: proof.poId, type: "single" as const, billingNumber: proof.billingNumber }
      : null;
  ...
  return (
    <div data-theme={theme} className="min-h-screen bg-base-200 font-mc">
      ...
      {/* ── Billing Modal — same component as the Billing page ── */}
      {billingOpen && billingItem && (
        <BillingDetailModal
          item={billingItem}
