...
export default function EditPage() {
  ...
  // Mobile tab state: "pdf" | "edit" | "chat"
  const [mobileTab, setMobileTab] = useState<"pdf" | "edit" | "chat">("pdf");
  // Summary collapse state (mobile only)
  ...
  /** PDF Panel — used in both desktop and mobile */
  ...
  /** Edit Panel — used in both desktop and mobile */
  const EditPanel = ({ className = "" }: { className?: string }) => (
    <div className={`bg-base-100 rounded-2xl border border-base-300 flex flex-col gap-4 p-4 overflow-hidden min-h-0 ${className}`}>
      ...
      <div className="lg:hidden">
        <button
          className="w-full flex items-center justify-between px-3 py-2.5 bg-base-200 rounded-xl border border-base-300 hover:border-primary transition-colors"
        ...
        >
          ...
          <div className="flex items-center gap-2">
            <span className="text-xs font-bold text-success tabular-nums">{formatPrice(grandTotal)}</span>
            <svg
              className={`w-3.5 h-3.5 text-base-content/40 transition-transform duration-200 ${summaryOpen ? "rotate-180" : ""}`}
      ...
      <div className="flex flex-col gap-2 flex-1 min-h-0">
        ...
        <div className="rounded-xl border border-base-300 overflow-hidden flex-1 min-h-0 flex flex-col">
          <div className="overflow-auto flex-1 min-h-0">
            <table className="table table-sm w-full min-w-[820px]">
  ...
  /** Chat Panel — used in both desktop and mobile */
  const ChatPanel = ({ className = "" }: { className?: string }) => (
    <div className={`bg-base-100 rounded-2xl border border-base-300 p-4 flex flex-col gap-3 ${className}`}>
      <div className="flex items-center justify-between">
        <span className="text-[10px] font-semibold tracking-[0.15em] uppercase text-base-content/40">
          AI Assistant
        </span>
    ...
    <>
      ...
      <div className="lg:hidden flex flex-col h-screen bg-base-200 overflow-hidden">
        ...
        <div className="flex items-center justify-between px-3 py-2.5 bg-base-100 border-b border-base-300 shrink-0">
          <button
            className="btn btn-ghost btn-xs h-7 min-h-0 gap-1 rounded-lg text-xs text-base-content/60"
            onClick={() => router.push("/test/rfq")}
          >
            <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <span className="text-xs font-semibold text-base-content/70 truncate max-w-[140px]">
            {form.rfq_number || "RFQ"}
          </span>
          <button
            className="btn btn-primary btn-xs h-7 min-h-0 rounded-lg gap-1 text-xs"
            onClick={handleSubmit}
            disabled={saving}
          >
        ...
        <div className="flex bg-base-100 border-b border-base-300 shrink-0">
          {tabs.map((tab) => (
            <button
              key={tab.key}
              className={`flex-1 flex flex-col items-center gap-0.5 py-2 text-[10px] font-semibold tracking-wider uppercase transition-colors
                ${mobileTab === tab.key
                  ? "text-primary border-b-2 border-primary"
                  : "text-base-content/40 border-b-2 border-transparent hover:text-base-content/60"
                }`}
        ...
        <div className="flex-1 overflow-hidden p-3">
          {mobileTab === "pdf" && (
            <PdfPanel className="h-full" />
          )}
          {mobileTab === "edit" && (
            <div className="h-full overflow-auto">
              <EditPanel className="min-h-full" />
            </div>
          )}
          {mobileTab === "chat" && (
      ...
      <div className="hidden lg:grid h-screen bg-base-200 p-4 gap-4 grid-cols-[42%_1fr] grid-rows-[1fr_380px] overflow-hidden">
        ...
        <div
          ref={pdfRef}
          className="bg-base-100 rounded-2xl border border-base-300 flex flex-col gap-3 p-4 row-span-2 overflow-hidden"
        >
          {/* Header */}
          <div className="flex items-center justify-between">
            <span className="text-[10px] font-semibold tracking-[0.15em] uppercase text-base-content/40">
        ...
        {/* RIGHT TOP: RFQ DATA (desktop — no summary dropdown) */}
        <div className="bg-base-100 rounded-2xl border border-base-300 flex flex-col gap-4 p-4 overflow-hidden min-h-0">
          <div className="flex items-center justify-between">
            ...
            <div className="flex items-center gap-2">
              <button className="btn btn-outline btn-sm gap-1.5 h-8 min-h-0 rounded-xl text-xs" onClick={() => router.push("/test/rfq")}>Cancel</button>
              <button className="btn btn-primary btn-sm gap-1.5 h-8 min-h-0 rounded-xl text-xs" onClick={handleSubmit} disabled={saving}>
                {saving ? <><span className="loading loading-spinner loading-xs"></span>Saving...</> : <><svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" /></svg>Save</>}
              </button>
            </div>
          </div>
                ...
                <table className="table table-sm w-full min-w-[820px]">
                      ...
                      form.line_items.map((item: any, i: number) => {
                        ...
                        return (
                          <tr key={i} className="border-b border-base-200 hover:bg-base-50 transition-colors group">
                            ...
                            <td className="py-1.5"><input type="text" inputMode="decimal" value={focusedLineField === `${i}-qty` ? String(item.quantity || "") : formatCurrency(item.quantity || 0)} onFocus={() => setFocusedLineField(`${i}-qty`)} onBlur={() => setFocusedLineField("")} onChange={(e) => handleLineChange(i, "quantity", e.target.value)} className="input input-sm h-8 w-full rounded-lg bg-transparent border-transparent hover:border-base-300 focus:border-primary focus:bg-base-100 transition-all text-xs text-center" /></td>
                            ...
                            <td className="py-1.5"><input type="text" inputMode="decimal" value={focusedLineField === `${i}-price` ? String(item.unit_price || "") : formatCurrency(item.unit_price || 0)} onFocus={() => setFocusedLineField(`${i}-price`)} onBlur={() => setFocusedLineField("")} onChange={(e) => handleLineChange(i, "unit_price", e.target.value)} className="input input-sm h-8 w-full rounded-lg bg-transparent border-transparent hover:border-base-300 focus:border-primary focus:bg-base-100 transition-all text-xs text-right" /></td>
                            <td className="text-right text-xs font-semibold text-primary py-1.5 tabular-nums">{formatPrice(total)}</td>
                            <td className="py-1.5">
                  ...
                  <tfoot className="sticky bottom-0 bg-base-200 border-t border-base-300">
                    <tr>
                      ...
                      <td className="text-right py-2.5"><span className="text-sm font-bold text-success tabular-nums">{formatPrice(grandTotal)}</span></td>
                      <td />
                    </tr>
                  </tfoot>
                </table>
        ...
        {/* RIGHT BOTTOM: Summary + Chat (desktop unchanged) */}
        <div className="grid grid-cols-2 gap-4 self-start overflow-hidden">
          {/* Summary card */}
          ...
          <div className="bg-base-100 rounded-2xl border border-base-300 p-4 flex flex-col gap-3 min-h-[380px]">
            <div className="flex items-center justify-between">
              <span className="text-[10px] font-semibold tracking-[0.15em] uppercase text-base-content/40">AI Assistant</span>
              <div className="flex items-center gap-1.5">
                <div className="w-1.5 h-1.5 rounded-full bg-success animate-pulse"></div>
                <span className="text-[10px] text-success font-medium">Online</span>
              </div>
            ...
            <div className="flex flex-col gap-2 flex-1 overflow-auto">
              <div className="self-start max-w-[88%]"><div className="px-3 py-2 bg-base-200 rounded-2xl rounded-tl-sm text-xs text-base-content/70 leading-relaxed">ระบบพร้อมสำหรับ AI Chat และ Document Assistant</div></div>
              <div className="self-end max-w-[88%]"><div className="px-3 py-2 bg-primary/12 rounded-2xl rounded-tr-sm text-xs text-primary leading-relaxed">RFQ นี้มียอดรวมเท่าไร?</div></div>
              <div className="self-start max-w-[88%]"><div className="px-3 py-2 bg-base-200 rounded-2xl rounded-tl-sm text-xs text-base-content/70 leading-relaxed">Grand Total: <span className="font-bold text-success tabular-nums">{formatPrice(grandTotal)}</span></div></div>
            </div>
            <div className="flex gap-2 pt-1 border-t border-base-200">
              <input type="text" placeholder="ถามเกี่ยวกับเอกสาร..." className="input input-bordered input-sm h-8 flex-1 rounded-xl text-xs bg-base-200 border-transparent focus:border-primary focus:bg-base-100 transition-colors" />
              <button className="btn btn-primary btn-sm h-8 min-h-0 rounded-xl px-3">
